...
func SetupRouter(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	r.GET("/health", healthHandler)     // For Render health check
	r.GET("/api/health", healthHandler) // For API health check
	...
	// Analytics endpoint (public for now, can be moved to protected later)
	...
	protected.Use(middleware.Auth(cfg.JWT.Secret))
	{
		...
		// Frontend-specific routes (aliases for compatibility)
		protected.GET("/me/transactions", txnCtl.GetTransactionHistory)
		protected.GET("/me/summary", sumCtl.Get)
		protected.GET("/me/profile", profCtl.Get)
	}
