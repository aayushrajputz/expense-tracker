...
func (s *ExpenseService) Create(e *models.Expense, uid uint) error {
	...
	// Parse the date string to time.Time (for future transaction integration)
	_, err = time.Parse("2006-01-02", e.Date)
	if err != nil {
		// If date parsing fails, continue (date validation handled by model)
		_ = err
	}

	// TODO: Create a corresponding transaction record for consistency
	// Temporarily disabled due to schema mismatch between old and new transaction models
...
func (s *ExpenseService) Update(id uint, uid uint, in *models.Expense) error {
	...
	// TODO: Update the corresponding transaction record
	// Temporarily disabled due to schema mismatch between old and new transaction models
	// transactionType := "debit"
	// if in.Type == "income" {
	// 	transactionType = "credit"
...
func (s *ExpenseService) Delete(id, uid uint) error {
	...
	// TODO: Delete the corresponding transaction record
	// Temporarily disabled due to schema mismatch between old and new transaction models
	// transactionID := fmt.Sprintf("MANUAL_%d", id)
